...
CATEGORIES = [
    ...
    "Pets", "Savings", "Taxes", "Insurance", "Internet", "Mobile", "Others",
]
...
def spending_projections(rows: list[dict], month_start: date, today: date) -> dict:
    ...
    Recurring expenses (mortgage, subscriptions, etc.) are treated as fixed
    monthly costs and excluded from the daily average calculation to avoid
    inflating the projection. The projected total = projected variable spending
    + actual recurring total already charged this month.
    """
    ...
    total_so_far = variable_total + recurring_total

    daily_avg = variable_total / days_elapsed
    weekly_avg = daily_avg * 7
    projected_total = (daily_avg * days_in_month) + recurring_total
